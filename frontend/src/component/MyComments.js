...
const MyComments = ({paintingId}) => {
    ...
    useEffect(()=>{
        GetAxios('/painting/'+paintingId+'/comment', {params:{}},(res)=>{
    ...
    const AddComment = () => {
        PostAxios('/painting/comment/',{contents:newComment, paintingId:paintingId},(res)=>{window.location.reload()})
    }

    const DeleteComment = (commentId) => {
        DeleteAxios('/painting/comment/'+ commentId, {params:{}}, (res)=>{window.location.reload()})
    }
    ...
    const onEdit = (commentId,contents) => {
        PutAxios('/painting/comment/'+commentId, {contents:contents},(res)=>{window.location.reload()})
    }
                        ...
                        {activeEdit === comment.commentId ?
                         <>
                          <div className="input_box">
                             <textarea className="input_comment" value={editComment} onChange={ChangeEditHandler}></textarea>
                             <MyButton text={'등록'} onClick={()=>onEdit(comment.commentId, editComment)}/> </div></>
                             : <></> }
                    </div>
                </div>
